...
{
	Vector4 resultx = Vector4(ix, iy, iz, iw) * (other.x);
	Vector4 resulty = Vector4(jx, jy, jz, jw) * (other.y);
	Vector4 resultz = Vector4(kx, ky, kz, kw) * (other.z);
	Vector4 resultw = Vector4(ox, oy, oz, ow) * (other.w);
	return (resultx + resulty + resultz + resultw);
}

Matrix4 Matrix4::operator*(Matrix4& other)
{
	/*Vector4 col1 = this * Vector4(other.ix, other.iy, other.iz, other.iw);
